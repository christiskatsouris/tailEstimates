...
$\textbf{(b).}$ To compute the elements of the VaR-DCoVaR risk matrix with both OLS and IVX estimates (as a user defined option) based on pairwise nonstationary quantile predictive regression models fitted on the nodes of the graph (coding procedure as per R Script compiled December 2021 at HPC of the University of Southampton)..

## Installation (under development)

The R package [‘tailEstimates’](https://github.com/christiskatsouris/tailEstimates) will be able to be installed from Github and [CRAN](https://cran.r-project.org/) soon.
...
The estimation methodology of the VaR-DCoVaR Risk matrix consists of the main novelity of the proposed framework which involves the construction of a large covariance-type matrix based on nodewise pairewise quantile predictive regression models. Furthermore, our proposed estimation methodology tackles the aspect of endogeneity. Endogeneity is one of the most studied problems in econometrics. As a matter of fact failure to address it generally results in biased estimates and therefore wrong conclusions. However, under the assumption of persistence predictors, endogeneity between the innovation sequence of the predictive regression and that of the nonstationary autoregressive model of predictors introduces such second-order biased effects. 
